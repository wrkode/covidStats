...
# plot cases_weekly against date for each country and scatter deaths_weekly
# Cleaning Data: using .replace to remove zero-value and substitute with NaN.
ax.plot(reportNl['dateRep'], reportNl['cases_weekly'].replace(0, np.nan), label= "NL", color='r')
ax.scatter(reportNl['dateRep'], reportNl['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o',label= "NL deaths_weekly", color='r')
ax.plot(reportIt['dateRep'], reportIt['cases_weekly'].replace(0, np.nan), label= "IT", color='b')
ax.scatter(reportIt['dateRep'], reportIt['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o', label= "IT deaths_weekly", color='b')
ax.plot(reportUs['dateRep'], reportUs['cases_weekly'].replace(0, np.nan), label= "US", color='g')
ax.scatter(reportUs['dateRep'], reportUs['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o',label= "US deaths_weekly", color='g')
ax.plot(reportUk['dateRep'], reportUk['cases_weekly'].replace(0, np.nan), label= "UK", color='orange')
ax.scatter(reportUk['dateRep'], reportUk['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o',label= "UK deaths_weekly", color='orange')
ax.plot(reportEs['dateRep'], reportEs['cases_weekly'].replace(0, np.nan), label= "ES", color='purple')
ax.scatter(reportEs['dateRep'], reportEs['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o',label= "ES deaths_weekly", color='purple')
ax.plot(reportEs['dateRep'], reportEs['cases_weekly'].replace(0, np.nan), label= "FR", color='gray')
ax.scatter(reportFr['dateRep'], reportFr['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o',label= "FR Deaths", color='gray')
ax.plot(reportDe['dateRep'], reportDe['cases_weekly'].replace(0, np.nan), label= "DE", color='black')
ax.scatter(reportDe['dateRep'], reportDe['deaths_weekly'].replace(0, np.nan), alpha=0.5, marker='o',label= "De deaths_weekly", color='black')
